...
async function loadStakes() {
  ...
  stakes.forEach((s, i) => {
    ...
    const startDate = s.startTime > 0 ? new Date(s.startTime * 1000).toLocaleDateString("th-TH") : "-";

    const claimTime = s.lastClaimTime + 15 * 24 * 60 * 60;
    const nextClaim = claimTime > 0 ? new Date(claimTime * 1000).toLocaleDateString("th-TH") : "-";

    const unlockTime = s.startTime + s.lockPeriod;
    const unlock = unlockTime > 0 ? new Date(unlockTime * 1000).toLocaleDateString("th-TH") : "-";
